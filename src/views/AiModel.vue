<template>
  ...
  >
    ...
    <div
      ...
      v-if="false && showContorlWidget && rotation === 0"
      @click.stop
    >
...
export default {
  ...
  methods: {
      ...
      if(this.skullEntity)
      {
        ...
        if(this.initialCameraPose?.cameraPosition && this.initialCameraPose?.focus)
        {
          ...
          cameraPos = new pc.Vec3(
            this.initialCameraPose.cameraPosition.x,
            this.initialCameraPose.cameraPosition.y,
            this.initialCameraPose.cameraPosition.z
          );
        } else {
          // 根据 AABB 和 FOV 计算相机位置
          const meshInstances = this.skullEntity.render?.meshInstances;
          if (meshInstances && meshInstances.length > 0) {
            ...
            // 根据 FOV 计算距离：distance = extent / tan(FOV/2) * scaleFactor
            const fovRad = (this.viewerControls.originalFov || 75) * Math.PI / 180;
            const distance = maxExtent / Math.tan(fovRad / 2) * 1.2;
            cameraPos = new pc.Vec3(center.x, center.y + maxExtent * 0.3, center.z + distance);
          } else {
            // 默认位置
            cameraPos = new pc.Vec3(center.x, center.y + 0.6, center.z + 2);
          }
        }

        // 使用 cameraControls.reset 重置视角
        this.cameraControls?.reset(center, cameraPos);
      }
      this.showControlsTimer();
    },
    getModelCenter() {
    ...
    initCamera() {
      if (!this.cameraControls || !this.cameraEntity || !this.skullEntity) return;
      ...
      // 直接设置相机位置，不调用 focusOnEntity（因为 GLB 模型不支持该方法）
      // 相机位置：模型中心后方 + 稍微抬高一点
      const cameraPos = new pc.Vec3(center.x, center.y + maxExtent * 0.3, center.z + distance);
      this.cameraControls.reset(aabb.center, cameraPos);
    },
    ...
    async initViewer() {
      ...
      try {
        ...
        // 限制缩放范围（min, max）
        this.cameraControls.zoomRange = new pc.Vec2(0.5, 10);
        this.updateCameraControls();
        // 添加环境光，让模型更亮
        ...
        fillLight.setEulerAngles(45, 180, 0);
    ...
    async getTargetModel() {
      // 从 URL 参数获取 GLB 模型地址，默认使用测试模型
      const modelUrl = this.$route.query.model || '/ai.glb' || "https://3dgs-web.metast.xyz/aimodel/ai.glb";
      if (!modelUrl) {
        showToast('未提供模型地址');
        return;
      }
      ...
      try {
        ...
        const response = await axios({
          ...
          onDownloadProgress: (progressEvent) => {
            if (progressEvent.total) {
              this.loading_progress = Math.round((progressEvent.loaded / progressEvent.total) * 100);
            }
          }
        });
        ...
        // 3. 加载 GLB 资产
        const asset = new pc.Asset('model', 'container', { url: fileUrl });
        ...
        asset.on('load', (loadedAsset) => {
          ...
          // 创建新的模型实体
          this.skullEntity = loadedAsset.resource.instantiateRenderEntity();
          console.log("模型加载完成", this.skullEntity)
          this.app.root.addChild(this.skullEntity);

          // 根据 AABB 居中模型并计算 gridEntity 位置
          const aabb = this.skullEntity.render?.meshInstances?.[0]?.aabb;
          console.log("aabb", aabb);
          if (aabb) {
            const distance = Math.max(aabb.halfExtents.x, aabb.halfExtents.y, aabb.halfExtents.z) * 2;
            const bottomPose = aabb.center.clone().sub(new pc.Vec3(0, aabb.halfExtents.y, 0));
            this.gridEntity.setPosition(bottomPose);
            this.cameraControls.zoomRange = new pc.Vec2(0, distance * 3);
          }
    ...
    async loadSplatAsset(fileName, fileUrl) {
      ...
      if(this.skullEntity) {
        ...
        else
        {
          ...
          if (vec) {
            // 根据向量计算相机位置
            cameraPos = center.clone().add(vec);
            // 限制距离范围
            let distance = vec.length();
            const maxDist = 10;
            if (distance > maxDist) {
              vec.normalize().mulScalar(maxDist);
              cameraPos = center.clone().add(vec);
            }
          } else {
            // 默认位置 - 根据 FOV 计算
            const halfExtents = aabb?.halfExtents || new pc.Vec3(1, 1, 1);
            const maxExtent = Math.max(halfExtents.x, halfExtents.y, halfExtents.z);
            const fovRad = (this.viewerControls.originalFov || 75) * Math.PI / 180;
            const distance = maxExtent / Math.tan(fovRad / 2) * 1.2;
            cameraPos = new pc.Vec3(center.x, center.y + maxExtent * 0.3, center.z + distance);
        ...
        // 限制相机距离范围
        const toCamera = cameraPos.clone().sub(center);
        let distance = toCamera.length();
        const minDistance = 0.5;
        const maxDistance = 10;
        distance = Math.max(minDistance, Math.min(distance, maxDistance));
        toCamera.normalize().mulScalar(distance);
        cameraPos = center.clone().add(toCamera);
        ...
        this.initialCameraPose = {
          ...
          focus: { x: center.x, y: center.y, z: center.z }
        };
