/**
 * 极简版 SOG 序列化：GSplatData → SOG 二进制Uint8Array
 * 移除所有非核心依赖，仅保留序列化逻辑，用JSZip打包，直接返回二进制数据
 */
...
// 引入JSZip（若CDN引入，可删除此import，直接使用全局JSZip）
...
const extractFromGSplatData = async (gsplatData, memberNames) => {
    ...
    // 从 GSplatData 提取属性（核心：用getProp获取原生数组）
...
const gsplatDataToSog = async (gsplatData, options = {}) => {
    ...
    try {
        ...
        // 5. 初始化WebGPU设备（GPU加速聚类）
        gpuDevice = await getGpuDevice();
        const device = gpuDevice.device;

        // 6. 初始化JSZip（替代自定义ZipWriter，打包SOG）
        const zip = new JSZip();

        // 7. 写入位置数据（双WebP）
        const means = await writeMeans(dataTable, indices, width, height);
        zip.file('means_l.webp', means.meansL);
        zip.file('means_u.webp', means.meansU);
        ...
        zip.file('scales.webp', scales.webp);

        // 10. 写入颜色+透明度（GPU聚类+WebP）
        const colors = await writeColors(dataTable, indices, width, height, iterations, device);
        zip.file('sh0.webp', colors.webp);

        // 11. 写入SH高阶系数（可选，GPU聚类+WebP）
        const dataSHBands = (() => {
        ...
        const shN = outputSHBands > 0 ?
            ...
            null;
        if (shN) {
            zip.file('shN_centroids.webp', shN.centroidsWebp);
            zip.file('shN_labels.webp', shN.labelsWebp);
        }

        // 12. 生成meta.json（SOG必需的元信息）
        const meta = {
            version: 2,
            asset: { generator: 'SuperSplat' }, // 简化版本，去掉version依赖
            count,
        ...
        // 13. 生成ZIP二进制（SOG本质），转为Uint8Array返回
        const sogBinary = await zip.generateAsync({
            type: 'uint8array',
            compression: 'DEFLATE', // 保持SOG的压缩规则
            compressionOptions: { level: 1 } // 平衡压缩比和速度
        });

        return sogBinary;
    } catch (error) {
        console.error('GSplatData转SOG失败：', error);
