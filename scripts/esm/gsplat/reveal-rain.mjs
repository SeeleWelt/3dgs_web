...
uniform bool uFallDownwards; // 新增：控制下落方向
...
void modifySplatCenter(inout vec3 center) {
    ...
    if (uTime < g_tLand) {
        ...
        // 垂直位移 - 根据uFallDownwards控制方向
        float verticalOffset = uDistance * (1.0 - fallProgress);
        center.y += uFallDownwards ? -verticalOffset : verticalOffset;
...
uniform uFallDownwards: bool; // 新增：控制下落方向
...
fn modifySplatCenter(center: ptr<function, vec3f>) {
    ...
    if (uniform.uTime < g_tLand) {
        ...
        // 垂直位移 - 根据uFallDownwards控制方向
        let verticalOffset = uniform.uDistance * (1.0 - fallProgress);
        if (uniform.uFallDownwards) {
 ...
 * rainScript.speed = 3;
 * rainScript.fallDownwards = true; // 启用从上到下下落
 */
class GsplatRevealRain extends GsplatShaderEffect {
    ...
    /**
     * Elevation above/below target position where splats start
     * @attribute
     * @range [0, 50]
     */
    distance = 30;
    ...
    /**
     * Whether rain falls downwards (from high to low Y) instead of upwards
     * @attribute
     * @type {boolean}
     */
    fallDownwards = true;
    ...
    updateEffect(effectTime, dt) {
        ...
        // 新增：传递下落方向参数到着色器
        this.setUniform('uFallDownwards', this.fallDownwards);
