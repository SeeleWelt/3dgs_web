...
export class AnnotationManager extends Script {
    ...
    /**
     * Returns per-annotation size multiplier.
     * @param {Annotation} annotation - The annotation
     * @returns {number}
     * @private
     */
    _getAnnotationSizeScale(annotation) {
        const size = Number(annotation?.size);
        return Number.isFinite(size) && size > 0 ? size : 1;
    }

    /**
     * Updates hotspot DOM size for a single annotation.
     * @param {Annotation} annotation - The annotation
     * @private
     */
    _applyAnnotationHotspotSize(annotation) {
        const resources = this._annotationResources.get(annotation);
        if (!resources) return;
        const scale = this._getAnnotationSizeScale(annotation);
        const size = this._hotspotSize * scale;
        resources.hotspotDom.style.width = `${size + 5}px`;
        resources.hotspotDom.style.height = `${size + 5}px`;
    }

    /**
     * Updates hotspot DOM sizes for all annotations.
     * @private
     */
    _updateAllHotspotSizes() {
        for (const [annotation] of this._annotationResources) {
            this._applyAnnotationHotspotSize(annotation);
        }
    }
    ...
    _showTooltip(annotation) {
        ...
        this._tooltipDom.style.transform = 'translate(0, 0)';
    ...
    _updateAnnotationPositions(annotation, resources, screenPos) {
        ...
        resources.hotspotDom.style.left = `${left}px`;
        resources.hotspotDom.style.top = `${top}px`;

        if (this._activeAnnotation === annotation) {
            const margin = 8;
            const tooltipWidth = this._tooltipDom.offsetWidth || 0;
            const tooltipHeight = this._tooltipDom.offsetHeight || 0;
            const targetX = left + 25;
            const targetY = top;

            const clampedX = Math.min(
                Math.max(targetX, margin),
                window.innerWidth - tooltipWidth - margin
            );
            const clampedY = Math.min(
                Math.max(targetY - tooltipHeight / 2, margin),
                window.innerHeight - tooltipHeight - margin
            );

            this._tooltipDom.style.left = `${clampedX}px`;
            this._tooltipDom.style.top = `${clampedY}px`;
        }
    }
    ...
    _updateAnnotationRotationAndScale(annotation) {
        ...
        annotation.entity.setLocalScale(worldSize * scale, worldSize * scale, worldSize * scale);
    }
    ...
    _registerAnnotation(annotation) {
        ...
        const onPointerDown = (e) => {
            ...
            e.stopPropagation();
            this.app.fire('annotation:click', annotation, e);
        ...
        const onPointerEnter = () => {
            if (annotation?.label === '0') return;
            if(this._annotationEditMode) return;
        ...
        eventHandles.push(annotation.on('size:set', () => {
            this._applyAnnotationHotspotSize(annotation);
    ...
    initialize() {
        ...
        const onEditModeChange = (enabled) => {
            this._annotationEditMode = enabled;
            if (enabled && this._activeAnnotation) {
                this._hideTooltip(this._activeAnnotation);
            }
        };
        this.app.on('annotation:editMode', onEditModeChange);
        ...
        this.once('destroy', () => {
            ...
            this.app.off('annotation:editMode', onEditModeChange);
...
export class Annotation extends Script {
    ...
    /**
     * Size multiplier for this hotspot (1 = default size).
     * @attribute
     * @title Size
     * @type {number}
     * @default 1
    ...
    set size(value) {
        ...
        this._size = Number.isFinite(next) && next > 0 ? next : 1;
    ...
    /**
     * Camera pose snapshot for this annotation.
     * shape: { position: {x,y,z}, focus: {x,y,z} }
     *
     * @attribute
     * @title Camera Pose
     * @type {object}
     */
    set cameraPose(value) {
        ...
        this.fire('cameraPose:set', this._cameraPose);
    }
